...
class DataPath extends Module with ALUConfig {
  ...
  for (i <- 0 until FETCH_NUM) {
    ...
    when(
      is_stage.io.forwarding_path_id(i).rs2 =/= TOT_PATH_NUM.U &
        !((inst.dec.param_b === BImm) & (inst.dec.path === PathALU))
    ) {
  ...
  for (j <- 0 until TOT_PATH_NUM) {
    ...
    if (j >= ALU_PATH_NUM)
      is_out(j).imm := issue_queue.io
        .inst(is_stage.io.path(j).order)
        .imm + is_out(j).b
    else
    ...
    is_out(j).is_delay_slot := is_stage.io.is_delay_slot(
      is_stage.io.path(j).order
    )
  }
  ...
  interrupt_reg.io.fu_is_delay_slot := fu_stage.io
    .sorted_fu_out(0)
    .is_delay_slot
