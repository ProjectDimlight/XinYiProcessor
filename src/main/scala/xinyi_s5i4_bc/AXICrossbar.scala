...
class AXICrossbar extends BlackBox {
  val io = IO(new Bundle {
    ...
    val s_axi_arregion = Input(UInt(12.W))
    ...
    val m_axi_awaddr   = Output(XLEN.W)
    ...
    val m_axi_awready  = Output(Bool())
    val m_axi_wdata    = Output(UInt(XLEN.W))
    val m_axi_wstrb    = Output(UInt())
