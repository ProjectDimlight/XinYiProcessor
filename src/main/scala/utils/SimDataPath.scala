...
class SimDataPath extends Module {
  ...
  if_stage.io.cache   <> sim_mem.io.icache_io
  if_stage.io.full    <> issue_queue.io.full
  if_stage.io.out     <> if_id_reg.io.if_out


  stall_frontend := sim_mem.io.icache_io.stall_req | issue_queue.io.full
  ...
  // FUs
  sim_mem.io.last_stall <> is_fu_reg.io.stalled
  ...
  def CreatePath(path_type: Int, j: Int, base: Int) = {
    if (path_type == 3) {
      ...
      fu.io.cache     <> sim_mem.io.dcache_io
  ...
  for (i <- 2 until 7) {
    interrupt(i) := DontCare
  }
  interrupt(7) := DontCare
